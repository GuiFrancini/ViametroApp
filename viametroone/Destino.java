...
public class Destino extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (String name : amarelos) {
            allButtons.add(name);
            colorMap.put(name, "#FFD700");
        }

        for (String name : verdes) {
            allButtons.add(name);
            colorMap.put(name, "#32CD32");
        }
        ...
        for (int i = 0; i < allButtons.size(); i += 3) {
            ...
            for (int j = i; j < i + 3 && j < allButtons.size(); j++) {
                ...
                btn.setOnClickListener(view -> {
                    ...
                    selectedButton = btn;
                    btn.setBackground(getRoundedDrawable("#2196F3"));
        ...
        btnAvancar.setOnClickListener(view -> {
            if (selectedButton != null) {
                String destinoSelecionado = selectedButton.getText().toString();

                // Recuperar a origem da intent anterior
                Intent intentOrigem = getIntent();
                String origemSelecionada = intentOrigem.getStringExtra("origem");

                // Atualizar destino no banco
                BancoHelper db = new BancoHelper(Destino.this);
                db.atualizarDestino(origemSelecionada, destinoSelecionado);

                // Enviar origem e destino para tela de cadastro
                Intent intent = new Intent(Destino.this, Cadastro.class);
                intent.putExtra("origem", origemSelecionada);
                intent.putExtra("destino", destinoSelecionado);
                startActivity(intent);
            } else {
                Toast.makeText(Destino.this, "Selecione uma estação!", Toast.LENGTH_SHORT).show();
            }
        });
    }
